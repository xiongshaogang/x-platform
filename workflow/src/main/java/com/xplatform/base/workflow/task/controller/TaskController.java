...
public class TaskController extends BaseController {
	...
	public AjaxJson endProcess(HttpServletRequest request,HttpServletResponse response){
		...
		if(StringUtil.isNotEmpty(businessKey)){
			ProcessInstanceEntity inst=this.processInstanceService.getProcessBusinessKey(businessKey);
			if(inst!=null && StringUtil.isNotEmpty(inst.getActInstId())){
				this.flowService.delProcessByInstanceId(inst.getActInstId(), memo, curUserId);
				message="流程结束成功!";
			}else{
				message="流程结束失败!";
			}
