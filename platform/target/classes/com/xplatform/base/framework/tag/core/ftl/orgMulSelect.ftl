<#if !customInput>
<input class="tag_orgMulsel_input" type="text" readonly="readonly" id="${displayId}" name="${displayName}" value="${displayValue}" title="${displayValue}" <#if extendDisplay??>${extendDisplay}</#if> />
</#if>

<button class="btn btn-pageselect-normal" type="button" onclick="goOrgMulSelectPage(this);" multiples="${multiples?string('true','false')}" 
...
orgCode="${orgCode}" orgId="${orgId}" containSelf="${containSelf?string('true','false')}" expandAll="${expandAll?string('true','false')}"
needOrg="${needOrg?string('true','false')}" needRole="${needRole?string('true','false')}" roleListUrl="${roleListUrl}" 
...
<#if extendButton??>${extendButton}</#if> >
