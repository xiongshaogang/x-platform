<#if !customInput>
<input class="tag_rolesel_input" type="text" readonly="readonly" id="${displayId}" name="${displayName}" value="${displayValue}" title="${displayValue}" <#if extendDisplay??>${extendDisplay}</#if> />
</#if>

<button class="btn btn-pageselect-normal" type="button" onclick="goRoleSelectPage(this);" multiples="${multiples?string('true','false')}"  
