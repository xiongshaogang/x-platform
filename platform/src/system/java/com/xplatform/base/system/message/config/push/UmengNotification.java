...
public abstract class UmengNotification {
	...
	protected static final HashSet<String> ROOT_KEYS = new HashSet<String>(Arrays.asList(new String[] { "appkey", "timestamp", "type", "device_tokens",
	...
	public AjaxJson send() throws Exception {
...
//		String result = HttpUtils.request(url);
		...
		if (status == 200) {
			...
			j.setSuccess(true);
			j.setMsg("Notification sent successfully.");
		} else {
			System.out.println("Failed to send the notification!");
			j.setSuccess(false);
			j.setMsg("Failed to send the notification!");
		}
		j.setObj(postBody);
		return j;
	}
