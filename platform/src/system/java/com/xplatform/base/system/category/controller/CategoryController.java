package com.xplatform.base.system.category.controller;
...
public class CategoryController extends BaseController{
	...
	public ModelAndView type(HttpServletRequest request) {
		return new ModelAndView("platform/system/category/categoryList");
	}
	...
	public void tree(TypeTreeVo type, HttpServletRequest request, HttpServletResponse response) throws BusinessException {
		...
		if (this.authorityService.isAdmin(map)) {//不是管理员，进行权限过滤
			List<CategoryEntity> typeList = categoryService.queryListByPorperty("parent.id", id);
			...
			List<CategoryTreeVo> typeList = new ArrayList<CategoryTreeVo>();
			typeList = categoryService.queryCategoryTree(user.getId(), id);
			//树的转换
			Map<String, String> propertyMapping = new HashMap<String, String>();
			propertyMapping.put(TreeMapper.PropertyType.ID.getValue(), "id");
			propertyMapping.put(TreeMapper.PropertyType.TEXT.getValue(), "name");
	...
	public void typeRoleTree(HttpServletRequest request, HttpServletResponse response) {
		...
		List<CategoryTreeVo> typeList=new ArrayList<CategoryTreeVo>();
		try {
			typeList = categoryService.queryTypeRoleTreeBySysType(map);
	...
	public void typeRoleTreeBySysTypeTree(HttpServletRequest request, HttpServletResponse response) {
		...
		List<CategoryTreeVo> typeList=new ArrayList<CategoryTreeVo>();
		try {
			typeList = categoryService.queryTypeRoleTreeBySysTypeTree(map);
		} catch (BusinessException e) {
	...
	public void queryUserTypeTree(HttpServletRequest request, HttpServletResponse response) {
		...
		List<CategoryTreeVo> typeList=new ArrayList<CategoryTreeVo>();
		try {
			typeList = categoryService.queryUserCategoryTree(request.getParameter("userId"), parentId);
		} catch (BusinessException e) {
			ExceptionUtil.printStackTraceAndLogger(e);
		}
		//树的转换
	...
	public void datagrid(CategoryEntity type, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		HttpSession session = ContextHolderUtils.getSession();
		Client client = ClientManager.getInstance().getClient(session.getId());
		UserEntity user = client.getUser();
		...
		if (this.authorityService.isAdmin(map)) {//不是管理员，进行权限过滤
			...
			Page<CategoryTreeVo> page = categoryService.findByPage(map, dataGrid);
	...
	@ResponseBody
	public AjaxJson saveOrUpdate(CategoryEntity type) {
		try {
			if (StringUtil.isNotEmpty(type.getId())) {

				message = "系统分类更新成功";
				categoryService.updateCategory(type);
			} else {
				message = "系统分类新增成功";
	 ...
	 * @return
	 */
	@RequestMapping(params = "categoryEdit")
	public ModelAndView typeEdit(CategoryEntity category, Model model) {
		...
		return new ModelAndView("platform/system/category/categoryEdit");
	}
	 ...
	 * @return
	 */
	@RequestMapping(params = "wirteCode")
	@ResponseBody
	public AjaxJson wirteCode(HttpServletRequest request) {
		try {
			...
			String code = PinyinUtil.getPinYinHeadChar(name);
			findByCode(code);
			code = wirteCode;
			message = code;
			result.setMsg(message);
	...
	public void findByCode(String code) {
		List<CategoryEntity> list = categoryService.queryListByPorperty("code", code);
		if (list.size() > 0) {
			int count = list.size();
	...
	public void queryUserFileTypeCheckedTree(HttpServletRequest request, HttpServletResponse response) {
		String parentId = "";
		List<CategoryTreeVo> typeVoList = new ArrayList<CategoryTreeVo>();
		if (StringUtil.isNotEmpty(request.getParameter("id"))) {
			parentId = request.getParameter("id");
			typeVoList = categoryService.queryUserFileCategoryTree(request.getParameter("userId"), parentId);
		} else {
			//如果首次进入,则只加载'资料分类'单个节点(因为它来自另一颗树,有很多同级节点)
			parentId = categoryService.queryCategoryIdByCondition("-1", "file");
			typeVoList = categoryService.queryUserFileCategoryRoot(request.getParameter("userId"), parentId);
		}
		//树的转换
		Map<String, String> propertyMapping = new HashMap<String, String>();
		propertyMapping.put(TreeMapper.PropertyType.ID.getValue(), "id");
	...
	public AjaxJson updateUserFileType(HttpServletRequest request, HttpServletResponse response) {
		...
		String userId = request.getParameter("userId");
		try {
			this.categoryService.updateUserFileCategory(checkedTypeIds, pIds, userId);
		} catch (BusinessException e) {
			// TODO Auto-generated catch block
	...
	public AjaxJson updateUserFileTypeAuthority(HttpServletRequest request, HttpServletResponse response) {
		...
		String userId = request.getParameter("userId");
		try {
			this.categoryService.updateUserFileCategoryAuthority(resourceIds, typeId, userId);
		} catch (BusinessException e) {
			// TODO Auto-generated catch block
	...
	public void queryRoleFileTypeCheckedTree(HttpServletRequest request, HttpServletResponse response) {
		...
		if (StringUtil.isNotEmpty(request.getParameter("id"))) {
			parentId = request.getParameter("id");
			typeVoList = categoryService.queryRoleFileCategoryTree(request.getParameter("roleId"), parentId);
		} else {
			//如果首次进入,则只加载'资料分类'单个节点(因为它来自另一颗树,有很多同级节点)
			parentId = categoryService.queryCategoryIdByCondition("-1", "file");
			typeVoList = categoryService.queryRoleFileCategoryRoot(request.getParameter("roleId"), parentId);
		}
		//树的转换
		Map<String, String> propertyMapping = new HashMap<String, String>();
		propertyMapping.put(TreeMapper.PropertyType.ID.getValue(), "id");
	...
	public AjaxJson updateRoleFileType(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			this.categoryService.updateRoleFileCategory(checkedTypeIds, pIds, roleId);
		} catch (BusinessException e) {
			// TODO Auto-generated catch block
	...
	public AjaxJson updateRoleFileTypeAuthority(HttpServletRequest request, HttpServletResponse response) {
		...
		String roleId = request.getParameter("roleId");
		try {
			this.categoryService.updateRoleFileCategoryAuthority(resourceIds, typeId, roleId);
		} catch (BusinessException e) {
			// TODO Auto-generated catch block
	...
	public void queryCommonAttachUploadTree(HttpServletRequest request, HttpServletResponse response) {
		...
		List<CategoryEntity> typeEntityList = new ArrayList<CategoryEntity>();
		List<TreeNode> treeList = new ArrayList<TreeNode>();
		List<CategoryTreeVo> typeVOList = new ArrayList<CategoryTreeVo>();
		//转换MAP设定
		Map<String, String> propertyMapping = new HashMap<String, String>();
		...
		if (StringUtil.isEmpty(id) && StringUtil.isNotEmpty(rootTreeCode)) {
			if ("rootType".equals(rootTreeCode)) {
				...
				id = categoryService.queryCategoryIdByCondition("-1", "file");
			} else {
				List<CategoryEntity> typeList1 = categoryService.queryListByPorperty("code", rootTreeCode);
		...
		if (onlyAuthority) {
			if (this.authorityService.isAdmin(queryMap)) {
				typeEntityList = categoryService.queryListByPorperty("parent.id", id);
				treeList = TreeMapper.buildJsonTree(typeEntityList, propertyMapping);
			} else { //不是管理员，进行权限过滤
				typeVOList = categoryService.queryViewableCategoryTree(queryMap);
	...
	public List<TreeGrid> getUserDataDirTreegrid(HttpServletRequest request, HttpServletResponse response) {
		...
		if (StringUtil.isNotEmpty(request.getParameter("id"))) {
			parentId = request.getParameter("id");
			typeVoList = categoryService.queryUserFileCategoryTree(userId, parentId);
		} else {
			//如果首次进入,则只加载'资料分类'单个节点(因为它来自另一颗树,有很多同级节点)
			parentId = categoryService.queryCategoryIdByCondition("-1", "file");
			typeVoList = categoryService.queryUserFileCategoryRoot(userId, parentId);
		}
		//查找公司资料管理下所有权限
		List<ResourceEntity> resourceList = resourceService.queryResourceListByModuleCode("dataManager");
		for(CategoryTreeVo typeTreeVo : typeVoList){
			//查找用户分类下所有的资源权限
	...
	public AjaxJson updateUserDataDirAndResource(HttpServletRequest request){
		...
		try{
			categoryService.updateUserFileCategory(dataDirIds, unSelParentIds, userId);
		...
		try{
			for(Entry<String, String> entry:entrySet){
				...
				categoryService.updateUserFileCategoryAuthority(resourceIds, dataDirId, userId);
			}
	...
	public AjaxJson updateRoleDataDirAndResource(HttpServletRequest request){
		...
		String dataDirResourceIds = request.getParameter("dataDirResourceIds");
		try{
			categoryService.updateRoleFileCategory(dataDirIds, unSelParentIds, roleId);
			result.setMsg("更新资料文件夹权限成功");
		}catch(Exception e){
			result.setMsg("更新资料文件夹权限失败");
			return result;
		...
		try{
			for(Entry<String, String> entry : entrySet){
				String dataDirId = entry.getKey();
				String resourceIds = entry.getValue();
				categoryService.updateRoleFileCategoryAuthority(resourceIds, dataDirId, roleId);
			}
			result.setMsg("更新资料文件夹及操作按钮权限成功");
		}catch(Exception e){
			result.setMsg("更新资料文件夹及操作按钮权限失败");
	...
	public void setTypeUserService(TypeUserService typeUserService) {
