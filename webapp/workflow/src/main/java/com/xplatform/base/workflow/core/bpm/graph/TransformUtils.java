...
public class TransformUtils {
	...
	protected static Logger logger = LoggerFactory
			.getLogger(TransformUtils.class);
	...
	private static OrthogonalType calcOrthogonalLinkRelativPosition(
			Shape source, Shape target, Port startPort, Port endPort) {
		double startx = source.getX() + source.getW() * startPort.getX()
				+ startPort.getHorizontalOffset();
		double starty = source.getY() + source.getH() * startPort.getY()
				+ startPort.getVerticalOffset();
		Point start = new Point((float) startx, (float) starty);

		double endx = target.getX() + target.getW() * endPort.getX()
				+ endPort.getHorizontalOffset();
		double endy = target.getY() + target.getH() * endPort.getY()
		...
		Point sprb = new Point(source.getX() + source.getW(), source.getY()
				+ source.getH());
		Point splb = new Point(source.getX(), source.getY() + source.getH());
		Point spc = new Point(source.getX() + source.getW() / 2.0F, source
				.getY()
				+ source.getH() / 2.0F);

		Point tplt = new Point(target.getX(), target.getY());
		Point tprt = new Point(target.getX() + target.getW(), target.getY());
		Point tprb = new Point(target.getX() + target.getW(), target.getY()
				+ target.getH());
		Point tplb = new Point(target.getX(), target.getY() + target.getH());
		Point tpc = new Point(target.getX() + target.getW() / 2.0F, target
				.getY()
				+ target.getH() / 2.0F);
	...
	private static boolean isInsideTriangle(Point A, Point B, Point C, Point P) {
		float ab = (A.getX() - P.getX()) * (B.getY() - P.getY())
				- (B.getX() - P.getX()) * (A.getY() - P.getY());
		float bc = (B.getX() - P.getX()) * (C.getY() - P.getY())
				- (C.getX() - P.getX()) * (B.getY() - P.getY());
		float ca = (C.getX() - P.getX()) * (A.getY() - P.getY())
				- (A.getX() - P.getX()) * (C.getY() - P.getY());

		return ((ab >= 0.0F) && (bc >= 0.0F))
				|| ((ab <= 0.0F) && (bc <= 0.0F) && (((bc >= 0.0F) && (ca >= 0.0F)) || ((bc <= 0.0F) && (ca <= 0.0F))));
	}
	...
	public static ArrayList<Point> caculateTopTop(Shape fromShape,
