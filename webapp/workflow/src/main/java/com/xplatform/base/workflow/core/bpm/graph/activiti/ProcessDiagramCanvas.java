...
public class ProcessDiagramCanvas {
	...
	protected static Color START_COLOR = new Color(113, 146, 75);//开始结点颜色
	...
	protected static Color TASK_COLOR = new Color(241,241,241);//任务背景颜色
	...
	static {
		try {
			USERTASK_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("workflow/graph/image/user.png"));

			SCRIPTTASK_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("workflow/graph/image/script.png"));

			SERVICETASK_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("workflow/graph/image/service.png"));

			RECEIVETASK_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("workflow/graph/image/receive.png"));

			SENDTASK_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("workflow/graph/image/send.png"));

			MANUALTASK_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("workflow/graph/image/manual.png"));

			TIMER_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("workflow/graph/image/timer.png"));

			ERROR_THROW_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("workflow/graph/image/error_throw.png"));

			ERROR_CATCH_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("workflow/graph/image/error_catch.png"));

			CALLACTIVITY_IMAGE = ImageIO
					.read(ReflectUtil
							.getResourceAsStream("workflow/graph/image/call_activity.png"));

			colorsMap.put(TaskOpinionEntity.STATUS_INIT, Color.darkGray);
			colorsMap.put(TaskOpinionEntity.STATUS_AGREE, Color.green);
			colorsMap.put(TaskOpinionEntity.STATUS_ABANDON, Color.orange);
			colorsMap.put(TaskOpinionEntity.STATUS_CHECKING, Color.red);
			colorsMap.put(TaskOpinionEntity.STATUS_REFUSE, Color.blue);
			colorsMap.put(TaskOpinionEntity.STATUS_REJECT, new Color(138, 9, 2));
			colorsMap.put(TaskOpinionEntity.STATUS_RECOVER, new Color(2, 59, 98));
			colorsMap.put(TaskOpinionEntity.STATUS_PASSED, new Color(51, 136, 72));
			colorsMap.put(TaskOpinionEntity.STATUS_NOT_PASSED, new Color(130, 183,
					215));
	...
	public void drawSequenceflow(List<Point2D.Double> points) {
		for (int i = 0; i < points.size() - 1; i++) {
			Line2D.Double line = new Line2D.Double(
					...
					((Point2D.Double) points.get(i)).getY(),
					((Point2D.Double) points.get(i + 1)).getX(),
					((Point2D.Double) points.get(i + 1)).getY());
			this.g.draw(line);
			if (i == points.size() - 2)
				drawArrowHead(line);
		}
	...
	public void drawSequenceflowWidthLabel(BPMNEdge bpmnEdge) {
		...
		drawSequenceflowLabel(bpmnEdge.getName(), (int) bpmnEdge.getMidpoint()
				.getX(), (int) bpmnEdge.getMidpoint().getY(), flag);
	}
	...
	public void drawSequenceflowLabel(String name, int x, int y, int flag) {
		...
		case 1:
			...
			break;
		case 2:
			drawX = x - this.g.getFontMetrics().stringWidth(name)
					- this.g.getFontMetrics().getHeight() / 2;
	...
	private String[] fitTextToWidthAndBreak(String original, int width) {
		...
		} while ((this.fontMetrics.stringWidth(line2 + "...") > maxWidth)
				&& (line2.length() > 0));
	...
	public void drawUserTask(String name, int x, int y, int width, int height) {
		...
		this.g.drawImage(USERTASK_IMAGE, ix, iy, ICON_SIZE, ICON_SIZE, null);
	}
	...
	public void drawCollapsedCallActivity(String name, int x, int y, int width,
			int height) {
		drawCollapsedTask(name, x, y, width, height, false);
		this.g.drawImage(CALLACTIVITY_IMAGE, x + 7, y + 7, ICON_SIZE,
				ICON_SIZE, null);
	}
	...
	public void drawHighLight(int x, int y, int width, int height, Short status) {
		...
		if (status != TaskOpinionEntity.STATUS_INIT)
			this.g.setStroke(THICK_TASK_BORDER_STROKE);
		else {
	...
	public void drawHPool(String name, int x, int y, int width, int height) {
		...
		RoundRectangle2D roundRectangle2D = new RoundRectangle2D.Double(x, y,
				20.0D, height, 0.0D, 0.0D);
		...
		this.g.drawString(name, textX - textLen,
				textY + this.fontMetrics.getHeight() / 2);
	...
	public static Color getColor(String colorString) {
		Properties properties = (Properties) ApplicationContextUtil.getBean("configproperties");
		...
		if ((!"".equals(tempCOLOR)) && (tempCOLOR != null)) {
			...
			color = new Color(Integer.parseInt(COLOR_Strings[0]),
					Integer.parseInt(COLOR_Strings[1]),
