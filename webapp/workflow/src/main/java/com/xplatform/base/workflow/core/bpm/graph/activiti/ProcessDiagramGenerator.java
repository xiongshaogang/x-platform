...
public class ProcessDiagramGenerator {
	...
	static {
		graphDrawInstructions.put(BPMNShapType.StartEvent,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawNoneStartEvent(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
					}
				});
		graphDrawInstructions.put(BPMNShapType.ErrorEvent,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawErrorEndEvent(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
		...
		graphDrawInstructions.put(BPMNShapType.EndEvent,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawNoneEndEvent(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
		...
		graphDrawInstructions.put(BPMNShapType.CancelEvent,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawErrorEndEvent(shap.getName(),
		...
		graphDrawInstructions.put(BPMNShapType.Task,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawTask(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
		...
		graphDrawInstructions.put(BPMNShapType.UserTask,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawUserTask(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
		...
		graphDrawInstructions.put(BPMNShapType.ScriptTask,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawScriptTask(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
		...
		graphDrawInstructions.put(BPMNShapType.ServiceTask,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawServiceTask(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
		...
		graphDrawInstructions.put(BPMNShapType.ReceiveTask,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawReceiveTask(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
		...
		graphDrawInstructions.put(BPMNShapType.SendTask,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawSendTask(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
		...
		graphDrawInstructions.put(BPMNShapType.ManualTask,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawManualTask(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
		...
		graphDrawInstructions.put(BPMNShapType.ExclusiveGateway,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawExclusiveGateway(
								shap.getName(), (int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
					}
				});
		graphDrawInstructions.put(BPMNShapType.InclusiveGateway,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
		...
		graphDrawInstructions.put(BPMNShapType.ParallelGateway,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
		...
		graphDrawInstructions.put(BPMNShapType.SubProcess,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						if ((shap.isExpanded() != null)
								&& (!shap.isExpanded().booleanValue()))
		...
		graphDrawInstructions.put(BPMNShapType.CallActivity,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
		...
		graphDrawInstructions.put(BPMNShapType.HPool,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawHPool(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
		...
		graphDrawInstructions.put(BPMNShapType.HLane,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawHLane(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
		...
		graphDrawInstructions.put(BPMNShapType.VPool,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawVPool(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
		...
		graphDrawInstructions.put(BPMNShapType.VLane,
				new GraphDrawInstruction() {
					public void draw(ProcessDiagramCanvas processDiagramCanvas,
							BPMNShap shap) {
						processDiagramCanvas.drawVLane(shap.getName(),
								(int) Math.round(shap.getX()),
								(int) Math.round(shap.getY()),
								(int) Math.round(shap.getWidth()),
								(int) Math.round(shap.getHeight()));
	...
	public static ProcessDiagramCanvas initProcessDiagramCanvas(
			List<BPMNShap> shaps, List<BPMNEdge> edges) {
	...
	public static ProcessDiagramCanvas generateDiagram(String bpmnXml,
			List<String> highLightedActivities) {
		List<BPMNShap> bpmnShaps = extractBPMNShap(bpmnXml);
		List<BPMNEdge> bpmnEdges = extractBPMNEdge(bpmnXml);
		ProcessDiagramCanvas processDiagramCanvas = initProcessDiagramCanvas(
				bpmnShaps, bpmnEdges);
	...
	public static ProcessDiagramCanvas generateDiagram(
			Map<String, String> highLightedActivities, String bpmnXml) {
		List<BPMNEdge> bpmnEdges = extractBPMNEdge(bpmnXml);
		List<BPMNShap> bpmnShaps = extractBPMNShap(bpmnXml);
		ProcessDiagramCanvas processDiagramCanvas = initProcessDiagramCanvas(
				bpmnShaps, bpmnEdges);
	...
	public static InputStream generateDiagram(String bpmnXml,
			Map<String, String> highLightedActivities, String imageType) {
		return generateDiagram(highLightedActivities, bpmnXml).generateImage(
				imageType);
	}

	public static InputStream generateDiagram(String bpmnXml, String imageType,
			List<String> highLightedActivities) {
		return generateDiagram(bpmnXml, highLightedActivities).generateImage(
				imageType);
	}

	public static void drawActivity(ProcessDiagramCanvas processDiagramCanvas,
			BPMNShap bpmnShap, List<String> highLightedActivities) {
		GraphDrawInstruction drawInstruction = (GraphDrawInstruction) graphDrawInstructions
				.get(bpmnShap.getType());
		if (drawInstruction != null) {
			...
			if (properties != null) {
				isSequential = (String) bpmnShap.getProperties().get(
						"isSequential");
			}
			...
			processDiagramCanvas.drawActivityMarkers(
					(int) Math.round(bpmnShap.getX()),
					(int) Math.round(bpmnShap.getY()),
					(int) Math.round(bpmnShap.getWidth()),
					(int) Math.round(bpmnShap.getHeight()),
	...
	public static void drawActivity(ProcessDiagramCanvas processDiagramCanvas,
		...
		if (drawInstruction != null) {
			...
			if (highLightedActivities.containsKey(bpmnShap.getBpmnElement())) {
				String color = (String) highLightedActivities.get(bpmnShap
						.getBpmnElement());
	...
	public static void drawSequenceFlows(
			ProcessDiagramCanvas processDiagramCanvas, List<BPMNEdge> bpmnEdges) {
		for (BPMNEdge bpmnEdge : bpmnEdges)
			processDiagramCanvas.drawSequenceflowWidthLabel(bpmnEdge);
	}

	/*
	 * private static Map<String, Short> getHighLightedMap( List<BpmProStatus>
	 * highLightedActivities) { Map map = new HashMap(); for (BpmProStatus
	 * status : highLightedActivities) { map.put(status.getNodeid(),
	 * status.getStatus()); } return map; }
	 */

	private static void drawHighLight(
			ProcessDiagramCanvas processDiagramCanvas, BPMNShap bpmnShap) {
		processDiagramCanvas.drawHighLight((int) Math.round(bpmnShap.getX()),
				(int) Math.round(bpmnShap.getY()),
				(int) Math.round(bpmnShap.getWidth()),
				(int) Math.round(bpmnShap.getHeight()));
	}
	...
	private static void drawHighLight(
		...
		processDiagramCanvas.drawHighLight((int) Math.round(bpmnShap.getX()),
				(int) Math.round(bpmnShap.getY()),
				(int) Math.round(bpmnShap.getWidth()),
				(int) Math.round(bpmnShap.getHeight()), color);
	}
	...
	private static void drawHighLight(
		...
		processDiagramCanvas.drawHighLight((int) Math.round(bpmnShap.getX()),
				(int) Math.round(bpmnShap.getY()),
				(int) Math.round(bpmnShap.getWidth()),
				(int) Math.round(bpmnShap.getHeight()), status);
	}
	...
	public static Point2D.Double[] caculateCanvasSize(List<BPMNShap> shaps,
			List<BPMNEdge> edges) {
		...
		for (BPMNEdge edge : edges) {
			...
			if (directionType == DirectionType.UpToDown) {
				...
				if (directionType == DirectionType.DownToUp) {
					labelMinX = midPoint.getX()
							- fontMetrics.stringWidth(label)
							- fontMetrics.getHeight() / 2;
					labelMinY = midPoint.getY() - fontMetrics.getHeight() / 2
					...
					if (directionType == DirectionType.LeftToRight) {
						...
						labelMinY = midPoint.getY() + fontMetrics.getHeight()
								- fontMetrics.getHeight();
					}
				}
			}
		...
		return new Point2D.Double[] { new Point2D.Double(minX, minY),
				new Point2D.Double(maxX, maxY) };
	}

	private static void shiftProcessDefinition(List<BPMNShap> shaps,
			List<BPMNEdge> edges, double x, double y) {
		for (BPMNShap shap : shaps) {
	...
	public static List<BPMNShap> extractBPMNShap(String bpmnXml) {
		...
		for (Element shap : shaps) {
			...
			bpmnShap.setChoreographyActivityShape(shap
					.attributeValue("choreographyActivityShape"));
			...
			bpmnShap.setParticipantBandKind(shap
					.attributeValue("participantBandKind"));

			Element bound = (Element) shap.selectSingleNode("./omgdc:Bounds");
			bpmnShap.setX(java.lang.Double.parseDouble(bound
					.attributeValue("x")));
			bpmnShap.setY(java.lang.Double.parseDouble(bound
					.attributeValue("y")));
			bpmnShap.setWidth(java.lang.Double.parseDouble(bound
					.attributeValue("width")));
			bpmnShap.setHeight(java.lang.Double.parseDouble(bound
					.attributeValue("height")));

			Element component = (Element) root.selectSingleNode("//*[@id='"
					+ bpmnShap.getBpmnElement() + "']");
			if (component != null) {
	...
	public static List<BPMNEdge> extractBPMNEdge(String bpmnXml) {
		...
		for (Element edge : edges) {
			...
			for (Element waypoint : waypoints) {
				double x = java.lang.Double.parseDouble(waypoint
						.attributeValue("x"));
				double y = java.lang.Double.parseDouble(waypoint
						.attributeValue("y"));
			...
			Element component = (Element) root
					.selectSingleNode("//sequenceFlow[@id='" + bpmnElement
							+ "']");
			...
			for (int i = 1; i < points.size(); i++) {
				lens.add(java.lang.Double.valueOf(Math.abs(
						((Point2D.Double) points.get(i - 1)).getX()- ((Point2D.Double) points.get(i)).getX())+ 
						Math.abs(((Point2D.Double) points.get(i - 1)).getY()- ((Point2D.Double) points.get(i)).getY())));
			}
			double halfLen = 0.0D;
			for (Iterator<java.lang.Double> localIterator3 = lens.iterator(); localIterator3.hasNext();) {
				double len = localIterator3.next().doubleValue();
				halfLen += len;
			}
			halfLen /= 2.0D;
			...
			/*for (int i = 0; i < lens.size(); i++) {
		        accumulativeLen += ((Double)lens.get(i)).doubleValue();
			...
			for (int j = 0; j < lens.size(); j++) {
				accumulativeLen += ((java.lang.Double) lens.get(j))
						.doubleValue();
				if (accumulativeLen > halfLen)
					i = j;
				break;
	...
	private static BPMNShap setBPMNShapProperties(Element component,
			BPMNShap bpmnShap) {
		...
		if ((type == BPMNShapType.Task) || (type == BPMNShapType.ScriptTask)
			...
			if (multiInstanceLoopCharacteristics != null) {
				String isSequential = multiInstanceLoopCharacteristics
						.attributeValue("isSequential");
				properties.put("isSequential", isSequential);
			}
		}

		if (type == BPMNShapType.ErrorEvent) {
			Element errorEventDefinition = (Element) component
					.selectSingleNode("errorEventDefinition");
	...
	public static BPMNShapType getBPMNShapType(Element component) {
		...
		if (component.getName().equals("startEvent")) {
			...
			Element process = (Element) root.selectSingleNode("//*[@id='"
					+ processRef + "']");
			if (process.element("laneSet") != null) {
				Element shap = (Element) root
						.selectSingleNode("//*[@bpmnElement='" + id + "']");
				String isHorizontal = shap.attributeValue("isHorizontal");
				if ((isHorizontal != null)
						&& (isHorizontal.equalsIgnoreCase("false")))
			...
			String id = component.attributeValue("id");
			Element root = component.getDocument().getRootElement();
			Element shap = (Element) root.selectSingleNode("//*[@bpmnElement='"
					+ id + "']");
			String isHorizontal = shap.attributeValue("isHorizontal");
			if ((isHorizontal != null)
					&& (isHorizontal.equalsIgnoreCase("false")))
