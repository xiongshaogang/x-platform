      ...
      if (isBpmnResource(resourceName)) {
        ...
        if(TaskThreadService.getProcessCmd()!=null){
            ...
            if(isStartAssign==1){
				...
				try {
					actFlowDefXml = BpmUtil.transform((String)TaskThreadService.getProcessCmd().getVariables().get("actDefKey"), deployment.getName(), defXml);
					System.out.println(actFlowDefXml);
				}catch (Exception e) {
					e.printStackTrace();
				}
                try {
					bytes = actFlowDefXml.getBytes("UTF8");
