...
public class ${entityName}Controller extends BaseController {
	...
	public void datagrid(${entityName}Entity ${entityName?uncap_first},HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		try{
		//自定义追加查询条件
		if (StringUtil.isNotEmpty(request.getParameter("${cgformConfig.baseFormEntity.connect}"))) {
				cq.eq("${subG.entityName?lower_case}.id", request.getParameter("${cgformConfig.baseFormEntity.connect}"));
			} else {
				cq.eq("${subG.entityName?lower_case}.id", "-1");
			}
		}catch (Exception e) {
	...
	public AjaxJson save${entityName}(${entityName}Entity ${entityName?uncap_first},HttpServletRequest request) {
		try {
			if (StringUtil.isNotEmpty(${entityName?uncap_first}.getId())) {
				...
				if(StringUtil.isNotEmpty(${entityName?uncap_first}.get${subG.entityName?lower_case?cap_first}().getId())){
