...
public class ${entityName}ServiceImpl implements ${entityName}Service {
	...
	public String save(${entityName}Entity ${entityName?uncap_first}) throws BusinessException {
		String pk="";
		try {
			${entityName}Entity ${entityName?uncap_first}Entity = this.get(${entityName?uncap_first}.getParent().getId());
			//第一步，保存信息
			...
			if(${entityName?uncap_first}Entity != null){
				...
				${entityName?uncap_first}.setTreeIndex(${entityName?uncap_first}Entity.getTreeIndex()+","+pk);
				${entityName?uncap_first}.setLevel(${entityName?uncap_first}Entity.getLevel()+1);
				${entityName?uncap_first}.setIsLeaf("1");
				this.${entityName?uncap_first}Dao.merge(${entityName?uncap_first});
				//第三步，更新父节点的信息
				if("1".equals(${entityName?uncap_first}Entity.getIsLeaf())){//父节点是叶子节点
					${entityName?uncap_first}Entity.setIsLeaf("0");
					this.${entityName?uncap_first}Dao.merge(${entityName?uncap_first}Entity);
				}
			}else{
				${entityName?uncap_first}.setId(pk);
	...
	public void delete(String id) throws BusinessException {
		try {
			...
			//获取父节点
			if(${entityName?uncap_first}!=null && !StringUtil.isEmpty(${entityName?uncap_first}.getParent().getId()) && !StringUtil.equals(${entityName?uncap_first}.getParent().getId(), "-1")){
