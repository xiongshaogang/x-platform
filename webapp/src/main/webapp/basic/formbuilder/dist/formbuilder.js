...
(function() {
  ...
  FormbuilderCollection = (function(_super) {
    ...
    FormbuilderCollection.prototype.comparator = function(model) {
    	console.log("comparator:" +model.attributes.label+";" + model.indexInDOM());
  ...
  ViewFieldView = (function(_super) {
    	...
    	if(e){
    		...
    		if(e.changed.isTitle){
    			...
    			if(e.get("isShow")){
        			...
        			if($(_this.parentView.collection.where({isTitle: true})).length > 1){
        				...
        				if(confirm("已经选择了其他要素“显示到列表标题区域”，是否替换？")){
        					_this.parentView.collection.remove(e, {silent: true});
        					$(_this.parentView.collection.where({isTitle: true})).each(function(){
        						_this.parentView.collection.get(this).set({isTitle: false}, {silent: true});
        					});
        					_this.parentView.collection.add(e, {silent: true});
        				}else{
        					e.set({isTitle: false});
        				}
        			}
    		...
    		if(e.changed.isShow){
    			...
    			if(_this.parentView.collection.where({isShow: true}).length > 3){
    				e.set({isShow: false}); 
    				alert("最多允许三个要素勾选。");
    			}
    		...
    		if(e.changed.field_options){
    			if(e.changed.field_options.digits){
    				...
    				if(!regStr.test(e.changed.field_options.digits)){
    					...
    					e.set({field_options: {digits: ""}}, {silent: false});
    				}
    			}
    		}
    	}
    ...
    ViewFieldView.prototype.clear = function(e) {
      ...
      cb = function() {
        ...
        $(_this.parentView.collection.where({pcid: _this.model.get("cid")})).each(function(){
        	this.destroy();
        });
  ...
  BuilderView = (function(_super) {
    ...
    BuilderView.prototype.saveForm = function(e) {
      ...
      if(this.collection.where({isTitle: true, pcid: undefined}).length < 1){
    ...
    BuilderView.prototype.isAllHasOptions = function(cObj){
        ...
        $(cObj.collection.where({field_type: "checkboxes"})).each(function(){
        	var optStrLen = 0;
        ...
        $(cObj.collection.where({field_type: "radio"})).each(function(){
        	var optStrLen = 0;
        	if(this.get("field_options").options.length < 1){
        		return allHasOptions = false;
        	}else{
...
(function() {
  Formbuilder.registerField('text', {
    order: 0,
    view: "<input type='text' placeholder=\'请输入内容\' class='rf-size-<%= rf.get(Formbuilder.options.mappings.SIZE) %>' />",
...
(function() {
  Formbuilder.registerField('paragraph', {
    order: 5,
    view: "<textarea placeholder=\'请输入内容\' class='rf-size-<%= rf.get(Formbuilder.options.mappings.SIZE) %>'></textarea>",
    edit: "<%= Formbuilder.templates['edit/size']() %>\n<%= Formbuilder.templates['edit/min_max_length']() %>",
    addButton: "多行文字",
    defaultAttributes: function(attrs) {
      attrs.field_options.size = 'small';
...
(function() {
  Formbuilder.registerField('date', {
    ...
    view: "<input type=\'text\'><i class=\'fa fa-angle-right\'></i>",
...
(function() {
  Formbuilder.registerField('idcard', {
    order: 30,
    view: "<input type=\'text\' placeholder=\'请输入身份证号\' >",
    edit: "",
    addButton: "身份证号"
  });
...
(function() {
  Formbuilder.registerField('phone', {
    order: 60,
    view: "<input type='text' placeholder=\'请输入电话号码\' />\n<% if (units = rf.get(Formbuilder.options.mappings.UNITS)) { %>\n  <%= units %>\n<% } %>",
    //edit: "<%= Formbuilder.templates['edit/min_max']() %>\n<%= Formbuilder.templates['edit/units']() %>\n<%= Formbuilder.templates['edit/integer_only']() %>",
    edit: "",
    addButton: "电话"
  });
...
(function() {
  ...
  Formbuilder.registerField('email', {
    order: 40,
    view: "<input type='text' class='rf-size-<%= rf.get(Formbuilder.options.mappings.SIZE) %>' placeholder=\'请输入邮箱\' />",
    edit: "",
    addButton: "邮箱"
  });
...
var __t, __p = '', __e = _.escape;
with (obj) {
__p += '<label id=\'isMustLabel\'>\n  <input type=\'checkbox\' data-rv-checked=\'model.' +
((__t = ( Formbuilder.options.mappings.REQUIRED )) == null ? '' : __t) +
'\' />必填\n</label>'+
//添加要素属性，第三处
'<br>\n<label><input type=\'checkbox\' data-rv-checked=\'model.'+((__t = ( Formbuilder.options.mappings.ISPROVEEDIT )) == null ? '' : __t)+'\' />审批时可修改</label>'+
'<br>\n<label id=\'isShowLabel\'><input type=\'checkbox\' data-rv-checked=\'model.'+((__t = ( Formbuilder.options.mappings.ISSHOW )) == null ? '' : __t)+'\' />显示到列表内容区域</label>'+
'<br>\n<label id=\'isTitleLabel\'><input type=\'checkbox\' data-rv-checked=\'model.'+((__t = ( Formbuilder.options.mappings.ISTITLE )) == null ? '' : __t)+'\' />显示到列表标题区域</label>'+
'<br>\n<label class=\'hidden\'><input type=\'checkbox\' data-rv-checked=\'model.'+((__t = ( Formbuilder.options.mappings.ISCONNECTIONFIELD )) == null ? '' : __t)+'\' />是否关联任务字段</label>'+
'\n<!-- label>\n  <input type=\'checkbox\' data-rv-checked=\'model.' +
...
this["Formbuilder"]["templates"]["edit/include_time"] = function(obj) {
	...
	var __t, __p = '', __e = _.escape;
	with (obj) {
		__p += '<div class=\'fb-edit-section-header\'>包含时间</div>\n<label>\n  <input type=\'checkbox\' data-rv-checked=\'model.' +
		((__t = ( Formbuilder.options.mappings.INCLUDE_TIME )) == null ? '' : __t) +
		'\' />\n  包含时间\n</label>\n';
	}
	return __p
...
this["Formbuilder"]["templates"]["partials/edit_form"] = function(obj) {
	...
	with (obj) {
		...
		'<div class=\'form-field clearfix\'><input class=\'field-checkbox\' id=\'isStartAssign\' type=\'checkbox\'/ ><div class=\'field-label\'>动态选择审批人</div></div>'+
		'<div class=\'form-field clearfix\'><input class=\'field-checkbox\' id=\'notifyType\' type=\'checkbox\'/ ><div class=\'field-label\'>是否选择传阅人</div></div>'+
		'<div class=\'form-field clearfix\'><input class=\'field-checkbox\' id=\'isSharefolder\' type=\'checkbox\'/ ><div class=\'field-label\'>发布到共享文件夹</div></div>'+
		...
		'<div class=\'form-field clearfix\' id=\'field_tempUser\'><div class=\'field-label no-float\'>任务使用人：</div><div id=\'tpltPermPerson\' class=\'person-list without-arrow\'><ul class=\'list-style-none-h\'><li class=\'add-item\' data-type=\'tplt\' id=\'tpltPermSetBtn\' data-toggle=\'modal\' data-target=\'#templateModal\'><div><img src="basic/img/group_member_add.png" alt="增加"></div></li></ul></div></div>'+
		'<div id=\'aproPermPersonDiv\' class=\'form-field clearfix\'><div class=\'field-label no-float\'>审批人：</div><div id=\'aproPermPerson\' class=\'person-list\'><ul class=\'list-style-none-h\'><li class=\'add-item\' id=\'aproPermSetBtn\' data-type=\'apro\' data-toggle=\'modal\' data-target=\'#templateModal\'><div><img src="basic/img/group_member_add.png" alt="增加"></div></li></ul></div></div>'+
...
'<div class=\'fb-right-container\'>\n  <ul class=\'fb-tabs header\'>\n    '+
'<li class=\'active\'><a data-target=\'#addField\'>要素</a></li>\n    '+
