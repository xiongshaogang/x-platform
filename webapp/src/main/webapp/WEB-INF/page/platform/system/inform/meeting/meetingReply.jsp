	...
	if(needReply == '1' && replyContent == ''){
		var myButton = [{
				...
				handler : function() {
					...
					$("#btn_type").val("reply");
					saveObj("btn_sub","meetingFormobj");
				}
	    },{
		id : 'transpond',
		text : '转发',
		...
		handler : function() {
			...
			if(str.indexOf('email') != -1 && $("#mailConfigId").val() == ''){
		...
		addButton(getD($("#meetingFormobj")),myButton,1);
	}else{
		var myButton = [{
		id : 'transpond',
		text : '转发',
...
function showOffice(){
  ...
  <t:formvalid formid="meetingFormobj" gridId="meetingInformList" action="meetingController.do?saveReply" refresh="true">
		...
		<table cellpadding="0" cellspacing="1" class="formtable" style="margin-right: 100px;">
		 <tr >
			...
			<iframe src="messageController.do?printFile&id=${me.id }&name=meetingTemplate.doc"  style="width: 0;height: 0;padding:0px;border:0px;" id="office_iframe" name="office_iframe"></iframe>
			</td>
			</tr>
		   <tr>
