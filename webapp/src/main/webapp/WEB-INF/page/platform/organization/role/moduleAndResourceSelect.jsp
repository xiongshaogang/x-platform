...
<style>
#roleModuleAndResourcePanel .grid_toolbar_div {
	height: 0px;
}

#roleModuleAndResourcePanel .datagrid .panel-body {
	border: none;
}
</style>
<div class="easyui-layout" style="width: 99%; height: 99%; margin: 4px auto 0px auto">
	<div region="center" style="border: 0px;">
		<div class="easyui-panel" style="padding: 1px;" fit="true" border="true" id="roleModuleAndResourcePanel"
			data-options="
			title :'模块列表',
			tools : [ {
				iconCls : 'icon-save',
				handler : function() {
					saveRoleModuleAndResource();
				}
			},{
				iconCls : 'icon-reload',
				handler : function() {
					roleModuleAndResource.reload();
				}
			},{
				iconCls : 'icon-tip'
			} ]">

			<t:datagrid name="roleModuleAndResourceSelect" checkbox="false" autoLoadData="true" pagination="false" idField="id" fitColumns="true"
				actionUrl="roleController.do?getRoleModuleTreegrid&roleId=${roleId }" onLoadSuccess="roleModuleAndResource.loadSucess(row,data)" fit="true"
				queryMode="group" treegrid="true">
...
<script type="text/javascript">
	var roleModuleAndResource = {
		expandAll : function() {
			$("#roleModuleAndResourceSelect").treegrid("expandAll");
		},
		reload : function() {
			$("#roleModuleAndResourceSelect").treegrid("reload");
		},
		loadSucess : function(row, data) {
			for (var i = 0; i < data.length; i++) {
				...
				if (checked == "checked") {
					$("#roleModuleAndResourceSelect").treegrid("expandAll", data[i].id);
				}
			}
		}
	}
	$(document).ready(function() {
		$.parser.onComplete = function() {
			$("#roleModuleAndResourceSelect").treegrid("options").onExpand = function() {
				...
				$("input:checkbox[id^='module_']").off().click(function(e) {
					...
					var children = $("#roleModuleAndResourceSelect").treegrid("getChildren", selectedId);
					if (children) {
						if ($(this).attr("checked")) {
					...
					var parent = $("#roleModuleAndResourceSelect").treegrid("getParent", selectedId);
					while (parent) {
						...
						parent = $("#roleModuleAndResourceSelect").treegrid("getParent", parent.id);
					}
				});
			};
	...
	function saveRoleModuleAndResource() {
		//模块权限
		...
		for (var i = 0; i < selectedResource.length; i++) {
			...
			//截取模块资源id为  {moduleId}_{resourceId}
		...
		$.ajax({
			url : "roleController.do?updateRoleModuleAndResource",
			method : "post",
			cache : "fasle",
			data : "moduleIds=" + moduleIds + "&moduleResourceIds=" + moduleResourceIds + "&roleId=${roleId}",
