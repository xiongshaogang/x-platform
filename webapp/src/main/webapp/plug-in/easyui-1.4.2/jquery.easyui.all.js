...
(function($){
	...
	function createButton(target) {
		var opts = $.data(target, 'linkbutton').options;
		var t = $(target).empty();
		/*t.addClass('l-btn').removeClass('l-btn-plain l-btn-selected l-btn-plain-selected l-btn-outline');添加了换肤样式*/
...
//		param = param || {};
	...
	function appendNodes(target, param){
		...
		if (node.length == 0){
			...
			if (isLeaf(target, node[0])){
				...
				nodeIcon.removeClass('tree-file').addClass('tree-folder fa fa-folder-open');
	...
	function selectNode(target, nodeEl){
		...
		$(target).find('div.tree-node-selected').removeClass('tree-node-selected bg-themeprimary');
		$(nodeEl).addClass('tree-node-selected bg-themeprimary');
			...
			function getTreeData(depth, children){
				...
				for(var i=0; i<children.length; i++){
					...
					if (item.state == 'closed'){
						...
						cc.push('<span class="icon-color fa fa-folder ' + (item.iconCls?item.iconCls:'') + '"></span>');
					} else {
						if (item.children && item.children.length){
							cc.push('<span class="tree-hit tree-expanded"></span>');
							cc.push('<span class="icon-color fa fa-folder fa fa-folder-open ' + (item.iconCls?item.iconCls:'') + '"></span>');
						} else {
							cc.push('<span class="tree-indent"></span>');
							cc.push('<span class="icon-color fa fa-file-text ' + (item.iconCls?item.iconCls:'') + '"></span>');
							isFileNode = true;
						}
					}
					if (opts.checkbox){
 ...
 */
(function($){
	//因与bootstrap的tooltip冲突，故取消easyui的tooltip工具
	return;
...
(function($){
	...
	function setScrollers(container) {
		...
		/**
		 * 增加自定义的tab前后div框 xiaqiang begin 
		 * add by xiaqiang
		 */
		var divLeft=header.children("div.tabs-div-left");
		var divRight=header.children("div.tabs-div-right");
		divLeft._outerHeight(tHeight);
		divRight._outerHeight(tHeight);
		//计算tabs页加起来的宽度,fullWidth完整宽度
		var wrapWidth=0;
		wrapWidth=tabsWidth+parseInt(wrap.children("ul").css("padding-left"));
		/**
		 * 增加自定义的tab前后div框 xiaqiang end 
		 */
		var cWidth = header.width() - tool._outerWidth()-divLeft._outerWidth()-divRight._outerWidth();//改的
		...
		if (tabsWidth > cWidth) {
			...
			//加的
			sLeft.css({
				left:divLeft._outerWidth()
			});
			sRight.css({
				right:divRight._outerWidth()
			});
			divRight.css({
				right:0
			});
			if (opts.toolPosition == 'left'){
				tool.css({
					left: sLeft.outerWidth()+divLeft._outerWidth(),//改的
					right: ''
				});
				wrap.css({
					marginLeft: sLeft.outerWidth() + tool._outerWidth()+divLeft._outerWidth(),//改的
					marginRight: sRight._outerWidth()+divRight._outerWidth(),//改的
					width: cWidth - sLeft.outerWidth() - sRight.outerWidth()
				});
			} else {
				//如果设置了tabs居中 加的
				if(opts.tabsAlign=="center"){
					wrap.css({
						marginLeft : "auto",
						marginRight : "auto",
						width : wrapWidth
					...
					tool.css({
						left: '',
						right: sRight.outerWidth()+divRight._outerWidth()//改的
					});
					wrap.css({
						marginLeft: sLeft.outerWidth()+divLeft._outerWidth(),//改的
						marginRight: sRight.outerWidth() + tool._outerWidth()+divRight._outerWidth(),//改的
						width: cWidth - sLeft.outerWidth() - sRight.outerWidth()
					});
				}
			}
			...
			if (opts.toolPosition == 'left'){
				tool.css({
					left: divLeft._outerWidth(),//改的
					right: ''
				});
				wrap.css({
					marginLeft: tool._outerWidth()+divLeft._outerWidth(),//改的
					marginRight: 0+divRight._outerWidth(),//改的
					width: cWidth
				});
			} else {
				//加的
				if(opts.tabsAlign=="center"){
					...
					tool.css({
						left: '',
						right: 0+divRight._outerWidth()//改的
					});
					wrap.css({
						marginLeft: 0+divLeft._outerWidth(),//改的
						marginRight: tool._outerWidth()+divRight._outerWidth(),//改的
						width: cWidth
					});
				}
			}
	...
	function addTools(container){
		 ...
		 */
		if(opts.leftDiv){
			var leftDiv=$("<div id=\""+container.id+"_left\" class=\"tabs-div-left\"></div>");
			header.append(leftDiv);
			leftDiv.width(opts.leftDivWidth);
			if(opts.leftDivTitle){
				var leftDivTitleSpan=$("<span class=\"tabs-div-title\"></span>");
				leftDivTitleSpan.text();
				leftDiv.append(leftDivTitleSpan);
				leftDivTitleSpan.text(opts.leftDivTitle);
			}
		}
		if(opts.rightDiv){
			var rightDiv=$("<div id=\""+container.id+"_right\" class=\"tabs-div-right\"></div>");
			header.append(rightDiv);
			rightDiv.width(opts.rightDivWidth);
			if(opts.rightDivTitle){
				var rightDivTitleSpan=$("<span class=\"tabs-div-title\"></span>");
				rightDivTitleSpan.text();
				rightDiv.append(rightDivTitleSpan);
				rightDivTitleSpan.text(opts.rightDivTitle);
			}
			//如果配置默认生成关闭按钮
			if(opts.closeBtn){
				var closeBtn=$("<div class=\"close-btn\" onclick=\"closeD(getD($(this)));\"><i class=\"fa fa-times\"></i></div>");
				rightDiv.append(closeBtn);
			}

			//预设的流程按钮
			if(opts.flowButton){
				var flowButton=opts.flowButton;
				rightDiv.append(flowButton);
				$.parser.parse(rightDiv);
			}
		}
		if(opts.hBorderLeft){
			header.addClass("tabs-header-border-left");
		}
		if(opts.hBorderRight){
			header.addClass("tabs-header-border-right");
		}
		if(opts.hBorderTop){
			header.addClass("tabs-header-border-top");
		}
		if(opts.hBorderBottom){
			header.addClass("tabs-header-border-bottom");
		}
		/** 
		 * 增加自定义的tab前后div框 xiaqiang end 
		 */
...
//							val = row[field] || '&nbsp;';
						...
						if (field == opts.treeField){
							...
							if (row.state == 'closed'){
								cc.push('<span class="tree-hit tree-collapsed"></span>');
								cc.push('<span class="icon-color fa fa-folder ' + (row.iconCls?row.iconCls:'') + '"></span>');
							} else {
								if (row.children && row.children.length){
									cc.push('<span class="tree-hit tree-expanded"></span>');
									cc.push('<span class="icon-color fa fa-folder fa fa-folder-open ' + (row.iconCls?row.iconCls:'') + '"></span>');
								} else {
									cc.push('<span class="tree-indent"></span>');
									cc.push('<span class="icon-color fa fa-file-text ' + (row.iconCls?row.iconCls:'') + '"></span>');
								}
							}
...
(function($){
	...
	function buildCombo(target){
		...
		/** 
		 * textname同样指定到初始input上  begin 
		 * add by xiaqiang 
		 */
		var textname = $(target).attr("textname");
		if(textname){
			$(target).textbox("textbox").attr("name",textname);
		}
		/** textname同样指定到初始input上  end */
...
//	}
	...
	function setValues(target, values){
		...
		/** 
		 * 在设置选中值的时候,同时设置值给产生easyui的那个html元素(为了validform验证框架) begin 
		 * add by xiaqiang 
		 */
		var oldValue = $(target).attr("value");
		$(target).attr("value", values.join(opts.separator));
		if (oldValue != "" || values.join(opts.separator) != "") {
			$(target).blur();
		}
		/** 在设置选中值的时候,同时设置值给产生easyui的那个html元素(为了validform验证框架) end */
...
//					item = panel.children('div.combobox-item:visible:last');
	...
	function setValues(target, values, remainText){
		...
		if (!$.isArray(values)){values = values.split(opts.separator)}
		panel.find('div.combobox-item-selected').removeClass('combobox-item-selected bg-themeprimary');
		var vv = [], ss = [];
		for(var i=0; i<values.length; i++){
			var v = values[i];
			var s = v;
			opts.finder.getEl(target, v).addClass('combobox-item-selected bg-themeprimary');
...
//		retrieveValues(target);
	...
	$.fn.combotree.methods = {
		...
		clear: function(jq){
			return jq.each(function(){
				var tree = $.data(this, 'combotree').tree;
				tree.find('div.tree-node-selected').removeClass('tree-node-selected bg-themeprimary');
