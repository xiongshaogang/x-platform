...
function getIframeDocument(id){
	console.info(window.frames["iframe_" + id]);
	...
	$("#tab_div_key tbody tr").each(function(i) {
		$(':input', this).each(function() {
			...
			if (name != null && (name.indexOf("mainTable") >= 0
				...
				boo?$this.removeAttr("readonly"):$this.attr("readonly","readonly");
			}
		});
	});
}
...
function fix(type){
	for(var i=0;i<=$('#tab_div_'+type+' tr:last').find('td:last').index();i++){
  		$('#tab_div_'+type+'_title th').eq(i).css('width', $('#tab_div_'+type+' tr:last').find('td').eq(i).width());
 	}
 	$('#tab_div_'+type+'_title').css('width',$('#tab_div_'+type+' tr:last').width());
}
