...
(function(d, f, b) {
	...
	var a = function(l, n, k) {
		...
		l.each(function() {
			...
			o.delegate(":text,:password", "keypress", function(q) {
	...
	a.util = {
		...
		usePlugin : function(o, l, n, r) {
			...
			if (r != "addRule" && o.jqtransform && d.fn.jqTransSelect) {
				...
				if (o.jqtransform.selector) {
					s.find(o.jqtransform.selector).filter('input:submit, input:reset, input[type="button"]')
							.jqTransInputButton();
		...
		getNullmsg : function(o) {
			...
			var l = o[0].settings.label || ".Validform_label";/*
																 * 添加了为空报错所提示"请填写xxxx"中xxxx的来源,如下结构<td ><label
																 * class="Validform_label">字段类型</label></td><td ><input
																 * type="text"
																 * datatype="*"/></td>
																 * //增加的条件为obj.parent().prev().children(label).eq(0).text()
																 * //add by
																 * xiaqiang
																 * 2014.5.21
																 */
			l = n.siblings(l).eq(0).text() || n.parent().prev().children(l).eq(0).text()
					|| n.siblings().find(l).eq(0).text() || n.parent().siblings(l).eq(0).text()
					...
					if (!(t in a.util.dataType)) {
						...
						if (!m) {
							...
							for ( var B in a.util.dataType) {
								...
								if (m[1] === z[1]) {
									var w = a.util.dataType[B].toString(), r = w.match(/\/[mgi]*/g)[1].replace("/", ""), x = new RegExp(
											"\\{" + z[2] + "," + z[3] + "\\}", "g");
		...
		check : function(v, t, n) {
			...
			if (o.dragonfly && !d(this).data("cked") && a.util.isEmpty.call(d(this), k)
			...
			if (q && !a.util.isEmpty.call(d(this), k) && !n) {
				...
				var p = {
					...
					data : "param=" + encodeURIComponent(k) + "&name=" + encodeURIComponent(d(this).attr("name"))
							+ "&oldValue=" + encodeURIComponent(d(this).attr("oldValue")) + "&uniquemsg="
							+ encodeURIComponent(d(this).attr("uniquemsg") ? d(this).attr("uniquemsg") : '')
							+ "&entityName="
							+ encodeURIComponent(d(this).attr("entityName") ? d(this).attr("entityName") : '')/*
																												 * 兼容编辑时的唯一性校验所填加的oldValue
																												 * add
																												 * by
																												 * xiaqiang
																												 * 2014.5.27
	...
	function h(n, m) {
		var l = (d(window).width() - n.outerWidth()) / 2, k = (d(window).height() - n.outerHeight()) / 2, k = (document.documentElement.scrollTop ? document.documentElement.scrollTop
