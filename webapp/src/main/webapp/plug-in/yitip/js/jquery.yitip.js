...
(function($){
	...
	yijs.Yitip.prototype = {
		...
		setPosition : function(position){
			...
			if(_$applyTo.attr("class")){
				/* 兼容easyui的各combo类组件的使用本框架验证 */
				if(_$applyTo.attr("class").indexOf("combo-f")>=0){
					var comboSpan=$.data(_$applyTo[0], "combo").combo;
					if(comboSpan[0]){
						var positionInput=comboSpan.find("input.textbox-text");
						_$applyTo=positionInput;
					}
				}
				/* 兼容组织机构类组件的框架验证 */
				if(_$applyTo.attr("class").indexOf("orgs")>=0){
					if(_$applyTo.attr("relateDisplay")){
						var relateDisplay=_$applyTo.attr("relateDisplay");
						var positionInput=$("#"+relateDisplay);
						_$applyTo=positionInput;
					}
				}
			}
			...
			//设置提示框位置
			//2014.7.3 xiaqiang 加入对z-index的调整,dialog拖拽窗口时会发生每次z-index改变,这里需要yitip随之改变
			var validDiv=getD(this.$applyTo).parent();
			_$tip.addClass(position).css(_pos).css("z-index",validDiv.css("z-index"));
			this.pos = _pos;
		...
		show : function(delay,speed){
			...
			// 隐藏/显示速度
			var _speed=speed||_effect.speed;
			...
			if($.browser.msie && $.browser.version < 8){
				//IE8以下不带偏移效果
				_$t.stop().delay(_delay).fadeIn(_speed);
			}else{
				var validDiv=getD(this.$applyTo).parent();
				_$t.stop().delay(_delay).css(_startCss).show().animate(_endCss,_speed).css("z-index",validDiv.css("z-index"));
			}			
		 ...
		 * 隐藏提示框
		 */
		hide : function(delay,speed){
			var hideDestroy=this.options.hideDestroy;
			...
			if ($.browser.msie && $.browser.version < 8) {
				//IE8以下不带偏移效果
				_$t.stop().delay(_delay).fadeOut(_speed);
			}else{
				_$t.stop().delay(_delay).css(_startCss).animate(_endCss,_speed,function(){
					if(hideDestroy){
						_$t.remove();
					}else{
						_$t.hide();
	...
	$.fn.yitip.defaults = {
		...
		/**显示/隐藏效果*/
		effect : {
			show : {"speed":"fast","offest":10,"startOpacity":0,"endOpacity":1},
			hide : {"speed":"fast","offest":10,"startOpacity":1,"endOpacity":0}
		},
		/**提示框模板*/
