	...
	$.LinkDivList = function($div,options){
		...
		function drawLine(x0, y0, x1, y1, color) {
			...
			if (y0 == y1) // 画横线
			{
				...
				var css = {top:y0,left:x0,width:Math.abs(x1 - x0),background:color,height:1};
			...
			} else if (x0 == x1) // 画竖线
			{
				...
				var css = {top:y0,left:x0,height:Math.abs(y1 - y0),background:color,width:1};
				rs.push(getLineHtml(css));
			} else {
				var lx = x1 - x0
				var ly = y1 - y0
				...
				for ( var i = 0; i < l; i += 2) {
					...
					var css = {top:py,left:px,height:2,background:color,width:1};
					rs.push(getLineHtml(css));
				}				
			}
			return rs
		...
		function addComputing(item){
			...
			select.css({top:top-8,left:left+width+12+5,width:42});
			item.append(select);
		};
