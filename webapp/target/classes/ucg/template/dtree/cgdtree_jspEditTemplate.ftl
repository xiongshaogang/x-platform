...
<html>
 ...
 <body>
  <t:formvalid callback="rereflash" usePlugin="password" layout="table" action="${entityName?uncap_first}Controller.do?save${entityName}" >
	<#list columns as po>
		...
		<#if po.editShow == 'N' && po.fieldName == 'parentId'>
		<input id="parent" name="parent.id" type="hidden" value="${'$'}{${entityName?uncap_first}.parent.id }">
		</#if>
	</#list>		
