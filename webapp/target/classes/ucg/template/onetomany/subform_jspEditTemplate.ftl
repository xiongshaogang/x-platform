...
<%@include file="/context/mytags.jsp"%>
  ...
  //编写自定义JS代码
  $(function(){
     if($("#${mainG.entityName?lower_case}").val() == ""){
        $("#${mainG.entityName?lower_case}").val("${'$'}{param.${mainG.entityName?uncap_first}Id}");
     }
  });
  </script>
  <t:formvalid formid="${entityName?uncap_first}Formobj"  action="${entityName?uncap_first}Controller.do?save${entityName}"  gridId="${entityName?uncap_first}List">
	<#list columns as po>
		...
		<#if po.editShow == 'N' && fieldMeta[po.fieldName] == cgformConfig.baseFormEntity.connect?upper_case>
		<input id="${mainG.entityName?lower_case}" name="${mainG.entityName?lower_case}.id" type="hidden" value="${'$'}{${entityName?uncap_first}.${mainG.entityName?lower_case}.id }">
		</#if>
	</#list>		
