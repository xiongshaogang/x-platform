...
<%@include file="/context/mytags.jsp"%>
  <t:formvalid  callback="reflashTree" formid="${entityName?uncap_first}Formobj" layout="table" action="${entityName?uncap_first}Controller.do?save${entityName}"  gridId="${entityName?uncap_first}List">
	<#list columns as po>
		<#if po.editShow == 'N' && fieldMeta[po.fieldName] != cField?upper_case>
		<input id="${po.fieldName}" name="${po.fieldName}" type="hidden" value="${'$'}{${entityName?uncap_first}.${po.fieldName} }">
		</#if>
		<#if po.editShow == 'N' && fieldMeta[po.fieldName] == cField?upper_case>
		<input id="${cEntityName?uncap_first}" name="${cEntityName?uncap_first}.id" type="hidden" value="${'$'}{${entityName?uncap_first}.${cEntityName?uncap_first}.id }">
		</#if>
	</#list>		
   ...
   <script type="text/javascript">
	  $(function(){
			$("#${cEntityName?uncap_first}").val($("#nodeid").val());
	  });
  </script>
