package com.xplatform.base.framework.mybatis.plugin.rewrite;
...
public class MySqlSessionFactoryBean extends SqlSessionFactoryBean {
	...
	public void readXML(Resource configXML, final Element elementTypeAlias, final Element elementMapper, final Element elementSettings,
		...
		saxReader.addHandler("/configuration/settings/setting", new ElementHandler() {
			...
			public void onEnd(ElementPath path) {
				if (settings_flag) {
					...
					els.addAttribute("value", row.attributeValue("value"));
					row.detach();
					settings_flag = false;
				}
			}
		...
		saxReader.addHandler("/configuration/typeAliases/typeAlias", new ElementHandler() {
			public void onEnd(ElementPath path) {
				...
				els.addAttribute("type", row.attributeValue("type"));
				row.detach();
			}
				...
				if (plugins_flag) {
					...
					if (row.elements() != null) {
						...
						while (it.hasNext()) {
							...
							Element newEle = els.addElement("property");
							newEle.addAttribute("name", ele.attributeValue("name"));
							newEle.addAttribute("value", ele.attributeValue("value"));
						}
					}
					row.detach();
					plugins_flag = false;
		...
		saxReader.addHandler("/configuration/mappers/mapper", new ElementHandler() {
			...
			public void onEnd(ElementPath path) {
				...
				els.addAttribute("mapper", row.attributeValue("mapper"));
				els.addAttribute("resource", row.attributeValue("resource"));
				row.detach();
			}
	...
	protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
		...
		for (Resource configLocation : configLocations) {
			readXML(configLocation, elementTypeAlias, elementMapper, elementSettings, elementPlugins);
		}
