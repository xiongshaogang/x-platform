...
public abstract class GenericBaseCommonDao<T, PK extends Serializable>
	...
	public <T> List<T> findByObjectPropertys(Class<T> entityClass,
	 ...
	 * @return
	 */
	private <T> Criteria createCriteria(Class<T> entityClass, boolean isAsc,List<String> orderPorpertys,
	...
	public <T> List<T> findByPropertyisOrder(Class<T> entityClass,
		...
		Assert.hasText(propertyName);
		return createCriteria(entityClass, isAsc,orderPorpertys,Restrictions.eq(propertyName, value)).list();
	}
	...
	public PageList getPageList(final HqlQuery hqlQuery, final boolean needParameter) {
		...
		String toolBar = null;
		if (StringUtil.isNotEmpty(hqlQuery.getMyaction())) {
			toolBar = PagerUtil.getBar(hqlQuery.getMyaction(), allCounts, curPageNO, hqlQuery.getPageSize(), hqlQuery.getMap());
		}
