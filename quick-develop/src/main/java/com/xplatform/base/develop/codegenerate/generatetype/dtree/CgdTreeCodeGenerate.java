package com.xplatform.base.develop.codegenerate.generatetype.dtree;
...
public class CgdTreeCodeGenerate implements ICallBack {
	private static final Log log = LogFactory.getLog(CgdTreeCodeGenerate.class);
	...
	public CgdTreeCodeGenerate() {
	}

	public CgdTreeCodeGenerate(CreateFileProperty createFileProperty2,
	...
	public CgdTreeCodeGenerate(CreateFileProperty tableFileProperty,
			GenerateVo table, GenerateVo tree) {
		// TODO Auto-generated constructor stub
		this.entityName = table.getEntityName();
		this.entityPackage = table.getEntityPackage();
		this.tableName = table.getTableName();
		this.ftlDescription = table.getFtlDescription();
		FIELD_ROW_NUM = 1;
		createFileProperty = tableFileProperty;
		createFileProperty.setJspMode("01");
		this.primaryKeyPolicy = table.getPrimaryKeyPolicy();
		this.sequenceCode = "";
		this.cgformConfig = table;
		this.subG = tree;
	}
		...
		try {
			...
			for (GenerateFieldEntity cf : columns) {
				...
				if ("string".equalsIgnoreCase(type))
					cf.setFieldType("java.lang.String");
				else if ("Date".equalsIgnoreCase(type))
					cf.setFieldType("java.util.Date");
				else if ("double".equalsIgnoreCase(type))
					cf.setFieldType("java.lang.Double");
				else if ("int".equalsIgnoreCase(type))
					cf.setFieldType("java.lang.Integer");
				else if ("BigDecimal".equalsIgnoreCase(type))
					cf.setFieldType("java.math.BigDecimal");
				else if ("Text".equalsIgnoreCase(type))
					cf.setFieldType("java.lang.String");
				else if ("Blob".equalsIgnoreCase(type)) {
					cf.setFieldType("java.sql.Blob");
				}
	...
	public void generateToFile() throws TemplateException, IOException {
		...
		CgdTreeCodeFactory codeFactory = new CgdTreeCodeFactory();
		
		if(StringUtil.equals(this.cgformConfig.getBaseFormEntity().getShowType(), "tree")){
			codeFactory.setCallBack(new CgdTreeCodeGenerate(createFileProperty,this.cgformConfig));
			
	        this.entityPackage = this.entityPackage.replace(".", "/");
			if (createFileProperty.isJspFlag()) {
				codeFactory.invoke("dtree/cgdtree_jspEditTemplate.ftl", "jspEdit");
			}
			this.entityPackage = this.entityPackage.replace("/", ".");
			if (createFileProperty.isActionFlag()) {
				codeFactory.invoke("dtree/cgdtree_controllerTemplate.ftl", "controller");
			}
			if (createFileProperty.isServiceImplFlag()) {
				codeFactory.invoke("dtree/cgdtree_serviceImplTemplate.ftl", "serviceImpl");
			}
			if (createFileProperty.isServiceIFlag()) {
				codeFactory.invoke("dtree/cgdtree_serviceITemplate.ftl", "service");
			}
			if (createFileProperty.isEntityFlag()) {
				codeFactory.invoke("dtree/cgdtree_entityTemplate.ftl", "entity");
			}
			if(createFileProperty.isDaoFlag()){
				codeFactory.invoke("dtree/cgdtree_daoTemplate.ftl", "dao");
			}
			if(createFileProperty.isDaoImplFlag()){
				codeFactory.invoke("dtree/cgdtree_daoImplTemplate.ftl", "daoImpl");
			}
		}if(StringUtil.equals(this.cgformConfig.getBaseFormEntity().getShowType(), "table")){
			data.clear();
            codeFactory.setCallBack(new CgdTreeCodeGenerate(createFileProperty,this.cgformConfig,subG));
			
	        this.entityPackage = this.entityPackage.replace(".", "/");
			if (createFileProperty.isJspFlag()) {
				codeFactory.invoke("dtree/cgdtable_jspListTemplate.ftl", "jspList");
				codeFactory.invoke("dtree/cgdtable_jspEditTemplate.ftl", "jspEdit");
			}
			this.entityPackage = this.entityPackage.replace("/", ".");
			if (createFileProperty.isActionFlag()) {
				codeFactory.invoke("dtree/cgdtable_controllerTemplate.ftl", "controller");
			}
			if (createFileProperty.isServiceImplFlag()) {
			...
			if (createFileProperty.isEntityFlag()) {
				codeFactory.invoke("dtree/cgdtable_entityTemplate.ftl", "entity");
			}
			if(createFileProperty.isDaoFlag()){
