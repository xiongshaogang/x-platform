...
public class DbTableProcess {
	...
	public static void createOrDropTable(MetaDataEntity table, Session session) throws Exception {
		Template t;
		t = getConfig("/com/xplatform/base/develop/cgform/engine/hibernate").getTemplate("tableTemplate.ftl");
	...
	private static Object getRootMap(MetaDataEntity table,String dataType) {
		Map map = new HashMap();
		for(MetaDataFieldEntity field :table.getColumns()){
			field.setFieldDefault(judgeIsNumber(field.getFieldDefault()));
		}
		List<MetaDataFieldEntity> columns = table.getColumns();
	...
	//给表字段去除增加的单引号
	@SuppressWarnings("all")
	private static void recoverTable(MetaDataEntity table) {
		for(MetaDataFieldEntity field :table.getColumns()){
	...
	public  List<String> updateTable(MetaDataEntity table,Session session) throws DBException{
	 ...
	 * @throws Exception
	 */
	private static void createTable(String xml, MetaDataEntity table,
...
//       
	 ...
	 * @return
	 */
	public static Map<String, ColumnMeta> getColumnMetadataFormCgForm(MetaDataEntity table){
	 ...
	 * @return
	 */
	public static Map<String, String> getNewAndOldFieldName(MetaDataEntity table){
