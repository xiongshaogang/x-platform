package com.xplatform.base.develop.codegenerate.window;
...
public class CodeWindow extends JFrame {
	...
	public CodeWindow() {
		...
		final JLabel infolbl = new JLabel("提示:");
		final JLabel showlbl = new JLabel();
		final JLabel packagebl = new JLabel("包名（小写）：");
		final JTextField packagefld = new JTextField();
		final JLabel entitylbl = new JLabel("实体类名（首字母大写）：");
		final JTextField entityfld = new JTextField();
		final JLabel tablejbl = new JLabel("表名：");
		final JTextField tablefld = new JTextField(20);

		final JLabel tablekeyjbl = new JLabel("主键生成策略：");
		final JComboBox tablekeyfld = new JComboBox(this.planets);
		final JLabel sequence_lb = new JLabel("主键SEQUENCE：(oracle序列名)");
		final JTextField sequence_fld = new JTextField(20);

		final JLabel titlelbl = new JLabel("功能描述：");
		final JTextField titlefld = new JTextField();

		final JLabel fieldRowNumlbl = new JLabel("行字段数目：");
		final JTextField fieldRowNumfld = new JTextField();
		fieldRowNumfld.setText(Integer.toString(fieldRowNum));
		...
		confirmbtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					boolean tableexist = new UcgReadTable()
