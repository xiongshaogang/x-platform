package com.xplatform.base.develop.codegenerate.generatetype.stree;
...
public class CgsTreeCodeGenerate implements ICallBack {
	private static final Log log = LogFactory.getLog(CgsTreeCodeGenerate.class);
	...
	public CgsTreeCodeGenerate() {
	}

	public CgsTreeCodeGenerate(CreateFileProperty createFileProperty2,
	...
	public void generateToFile() throws TemplateException, IOException {
		...
		CgsTreeCodeFactory codeFactory = new CgsTreeCodeFactory();
		
		codeFactory.setCallBack(new CgsTreeCodeGenerate(createFileProperty,this.cgformConfig));
		
        this.entityPackage = this.entityPackage.replace(".", "/");
		if (createFileProperty.isJspFlag()) {
			codeFactory.invoke("stree/cgstree_jspListTemplate.ftl", "jspList");
			codeFactory.invoke("stree/cgstree_jspEditTemplate.ftl", "jspEdit");
		}
		this.entityPackage = this.entityPackage.replace("/", ".");
		if (createFileProperty.isActionFlag()) {
			codeFactory.invoke("stree/cgstree_controllerTemplate.ftl", "controller");
		}
		if (createFileProperty.isServiceImplFlag()) {
			codeFactory.invoke("stree/cgstree_serviceImplTemplate.ftl", "serviceImpl");
		}
		if (createFileProperty.isServiceIFlag()) {
			codeFactory.invoke("stree/cgstree_serviceITemplate.ftl", "service");
		}
		if (createFileProperty.isEntityFlag()) {
			codeFactory.invoke("stree/cgstree_entityTemplate.ftl", "entity");
		}
		if(createFileProperty.isDaoFlag()){
			codeFactory.invoke("stree/cgstree_daoTemplate.ftl", "dao");
		}
		if(createFileProperty.isDaoImplFlag()){
			codeFactory.invoke("stree/cgstree_daoImplTemplate.ftl", "daoImpl");
		}
